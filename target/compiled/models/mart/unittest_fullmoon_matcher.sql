...
WITH
            	"AIRBNB_DEV_fct_reviews" as (SELECT CAST(NULL AS NUMBER) AS LISTING_ID, CAST('2025-01-13' AS TIMESTAMP_NTZ(9)) AS REVIEW_DATE, CAST(NULL AS VARCHAR) AS REVIEWER_NAME, CAST(NULL AS VARCHAR) AS REVIEW_TEXT, CAST(NULL AS VARCHAR) AS REVIEW_SENTIMENT
UNION ALL
SELECT CAST(NULL AS NUMBER) AS LISTING_ID, CAST('2025-01-14' AS TIMESTAMP_NTZ(9)) AS REVIEW_DATE, CAST(NULL AS VARCHAR) AS REVIEWER_NAME, CAST(NULL AS VARCHAR) AS REVIEW_TEXT, CAST(NULL AS VARCHAR) AS REVIEW_SENTIMENT
UNION ALL
SELECT CAST(NULL AS NUMBER) AS LISTING_ID, CAST('2025-01-15' AS TIMESTAMP_NTZ(9)) AS REVIEW_DATE, CAST(NULL AS VARCHAR) AS REVIEWER_NAME, CAST(NULL AS VARCHAR) AS REVIEW_TEXT, CAST(NULL AS VARCHAR) AS REVIEW_SENTIMENT),
...
FROM
  ...
  ON (TO_DATE(r.review_date) = DATEADD(DAY, 1, fm.full_moon_date)))
        (SELECT REVIEW_DATE, IS_FULL_MOON, 'actual' AS actual_or_expected FROM "AIRBNB_DEV_mart_fullmoon_reviews_actual")
        UNION ALL
        (SELECT REVIEW_DATE, IS_FULL_MOON, 'expected' AS actual_or_expected FROM "AIRBNB_DEV_mart_fullmoon_reviews_expect")
        ORDER BY REVIEW_DATE, IS_FULL_MOON